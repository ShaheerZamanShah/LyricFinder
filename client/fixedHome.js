                      ...
                      {recsLoading ? (
                        ...
                        <div className="grid gap-4 grid-cols-2 md:grid-cols-3">
                          {result.song.recommendations.map(r => (
                            <button key={r.id} onClick={()=>!recSongLoading && handleRecClick(r)} className={`group relative p-3 rounded-lg text-left bg-gray-700/30 hover:bg-gray-700/50 transition shadow ${recSongLoading?'opacity-60 pointer-events-none':''}`} title={`View lyrics: ${r.title} ΓÇô ${r.artist}`}>
              ...
              <div className="fixed inset-0 z-40 flex items-center justify-center p-4" onClick={()=>setShowLyricsModal(false)}>
                ...
                <div className={`relative max-w-3xl w-full max-h-[90vh] rounded-2xl overflow-hidden shadow-2xl ${theme==='light'?'bg-white':'bg-gray-900'}`} onClick={e=>e.stopPropagation()}>
                  <div className="flex items-center justify-between px-5 py-4 border-b border-gray-600/30">
                    ...
                    <button onClick={()=>setShowLyricsModal(false)} className="p-2 rounded hover:bg-gray-600/30" aria-label="Close lyrics">Γ£ò</button>
                  </div>
